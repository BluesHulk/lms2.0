<template>
  <MyContainer>
    ...
    <div class="content">
      <MyTitle :title="'新增收款单'" icon="icon-shoukuandan" />
      <MyForm
        ref="formRef"
        labelPosition="left"
        :formItems="formItems"
  ...
  <MyDialog
    :className="'receipt-voucher-dialog'"
    :visible="modalShow"
    :title="modalTitle"
    :width="640"
    @onUpdateDialogStatus="onUpdateDialogStatus"
  >
    <div class="receipt-voucher-content">
      <MyForm
        ref="modalFormRef"
...
export default {
  ...
  setup() {
    ...
    getDictionaryList("COLLECTION_BUSINESS_TYPE", businessTypeList);
    ...
    const getDetail = () => {
      receiptVoucherApi.getDetail(id).then((res) => {
        if (res && res.code == 0) {
    ...
    const onModalShow = (type, record = {}) => {
      if (!formRecord.formData.customerId) {
        ElMessage.warning("请先选择收款对象");
        return;
      }
      modalRecord.modalTitle =
        type == "add" ? "添加收款明细单" : "编辑收款明细单";
    ...
    const submitForm = () => {
      ...
      const data = {
        ...formRecord.formData,
        receiptRecordQos: tableRecord.tableData,
      };
      data.payTime &&
        (data.payTime = moment(data.payTime).format("YYYY-MM-DD HH:mm:ss"));
      delete data.billTime;
      delete data.receiptRecords;

      receiptVoucherApi[type == "add" ? "onAdd" : "onEdit"](data)
        .then((res) => {
          if (res && res.code == 0) {
