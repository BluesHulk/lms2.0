...
export default {
  ...
  setup() {
    ...
    const getTableData = () => {
      ...
      receiptVoucherApi.getTableData(params).then((res) => {
        if (res && res.code == 0) {
    ...
    const onAuditStatusChange = (record) => {
      if (record.checkStatus == 0) {
        ElMessageBox.confirm(
          ...
          receiptVoucherApi.onAudit(record.id).then((res) => {
            if (res && res.code == 0) {
        ...
        ElMessageBox.confirm(
          ...
          receiptVoucherApi.onCancelAudit(record.id).then((res) => {
            if (res && res.code == 0) {
              ElMessage.success("操作成功！");
              getTableData();
            } else {
    ...
    const onDelete = (record) => {
      ElMessageBox.confirm(
        ...
        receiptVoucherApi.onDelete(record.id).then((res) => {
          if (res && res.code == 0) {
            ElMessage.success("操作成功！");
            getTableData();
          } else {
