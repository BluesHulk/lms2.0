...
export default {
  ...
  setup() {
    ...
    const tableColumns = [
      ...
      { label: "付款账户", prop: "collectAccount" },
    ];

    const getDetail = () => {
      paymentOrderApi.getDetail(route.query.id)
        .then(res => {
          if(res && res.code == 0) {
            res.data.checkStatusName = { 0: '未审核', 1: '已审核' }[res.data.checkStatus]
            detailRecord.detailData = res.data
            tableRecord.tableData = transformArrayData(res.data.paymentRecords, true, true)
          }
        })
    }
    getDetail();
    ...
    const onAuditStatusChange = () => {
      if (detailRecord.detailData.checkStatus == 0) {
        ElMessageBox.confirm(
          ...
          paymentOrderApi.onAudit(detailRecord.detailData.id).then((res) => {
            if (res && res.code == 0) {
        ...
        ElMessageBox.confirm(
          ...
          paymentOrderApi.onCancelAudit(detailRecord.detailData.id).then((res) => {
            if (res && res.code == 0) {
              ElMessage.success("操作成功！");
              getDetail();
            } else {
