<template>
  ...
  >
    <div class="content">
      <div class="text">
        应付单：<span class="num">{{ modalData.money }}</span
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const getTableData = () => {
      ...
      delete params.time;
      billsPayableApi.getDetailTableData(params).then((res) => {
        if (res && res.code == 0) {
    ...
    const tableColumns = [
      { label: "付款单号", prop: "id", fixed: "left", width: 160 },
      { label: "付款主体", prop: "tenantName" },
