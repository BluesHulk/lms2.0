...
export default {
  ...
  setup(props, { emit }) {
    ...
    const tableColumns = [
      ...
      { label: "收款对象", prop: "customerName", width: 160, fixed: "left" },
      { label: "收款交易类型", prop: "tradeTypeName", width: 160 },
    ...
    const onAuditStatusChange = () => {
      if (detailRecord.detailData.checkStatus == 0) {
        ElMessageBox.confirm(
          ...
          singleReceivableApi
            .onAudit(detailRecord.detailData.id)
            .then((res) => {
              if (res && res.code == 0) {
    ...
    const onVerification = (record) => {
      if (record.verifyStatus == 0) {
        ...
        ElMessageBox.confirm(
          ...
          singleReceivableApi.onCancelVerify(record.id).then((res) => {
            if (res && res.code == 0) {
              ElMessage.success("操作成功！");
              getDetail();
            } else {
