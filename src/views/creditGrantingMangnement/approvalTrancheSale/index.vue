  ...
  setup() {
    ...
    const onRebackMoney = (row, type) => {
      if (type == 'del') {
        ...
        dialogFromData.FormDataDia.newCreditTotalMoney =
          Number(row.oldCreditTotalMoney) + Number(row.changeCreditMoney)
        dialogFromData.FormDataDia.newCreditMoney =
          Number(row.oldCreditMoney) + Number(row.changeCreditMoney)
      } else if (type == 'check') {
        //审核
    ...
    const onFormItemChange = (data, paramName) => {
            ...
            .then((res) => {
              ...
              if (code == 0) {
                dialogFromData.FormDataDia.oldCreditTotalMoney =
                  data.creditTotalMoney //信用总金额
                dialogFromData.FormDataDia.oldCreditMoney = data.creditUseMoney //可用信用额度
                dialogFromData.FormDataDia.changeCreditMoney = 0
                dialogFromData.FormDataDia.creditId = data.id
                dialogFromData.FormDataDia.newCreditTotalMoney =
                  data.creditTotalMoney +
                  dialogFromData.FormDataDia.changeCreditMoney
                dialogFromData.FormDataDia.newCreditMoney =
                  data.creditUseMoney +
                  dialogFromData.FormDataDia.changeCreditMoney
              } else {
          ...
          creditApi
            ...
            .then((res) => {
              ...
              if (code == 0) {
                dialogFromData.FormDataDia.oldCreditTotalMoney =
                  data.creditTotalMoney
                dialogFromData.FormDataDia.oldCreditMoney = data.creditUseMoney
                dialogFromData.FormDataDia.changeCreditMoney = 0
                dialogFromData.FormDataDia.creditId = data.id
                dialogFromData.FormDataDia.newCreditTotalMoney =
                  data.creditTotalMoney +
      ...
      if (paramName == 'changeCreditMoney') {
        console.log(data, 'data')
        dialogFromData.FormDataDia.newCreditTotalMoney =
          Number(data.oldCreditTotalMoney) + Number(data.changeCreditMoney)
        dialogFromData.FormDataDia.newCreditMoney =
          Number(data.oldCreditMoney) + Number(data.changeCreditMoney)
      }
    }
    const getTenantList = () => {
