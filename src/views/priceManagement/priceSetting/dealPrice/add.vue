<template>
        ...
        <el-form ref="tableFrom" :model="tableDataFrom" :rules="tableRules">
          ...
          <template v-slot:productPriceHeader>
            ...
            <template v-slot:priceHeader>
              协议价格<span class="required-symbol">*</span>
            </template>
  ...
  <div class="price-list-footer">
    <el-button  @click="saveDrafts" :loading="btnLoading"
      >保存并关闭</el-button
    >
    <el-button
      type="primary"
  ...
  <MaterialModal :visible="modalShow" @onClose="onModalClose" 
...
export default defineComponent({
      ...
      if (paramName == "payType") {
       ...
       if (paramName == "siteId") {
        siteList.value.map((item) => {
          if (item.value == data.siteId) {
            state.formData.siteName = item.label;
          }
        });
      }
    };
 const modalRecord = reactive({
    ...
    const submitForm = () => {
      ...
      priceSettingApi[type == "add" ||type == "copy"? "onAdd" : "onEdit"](data)
        .then((res) => {
          if (res && res.code == 0) {
    ...
    getuserbussiness();
    // 获取客户地址
    const getAddressList = (customerId) => {
      const params = {
        clientId:customerId
      }
      priceSettingApi.getAddress(params).then((res) => {
        if (res && res.code == 0) {
          res.data.map((item) => {
    ...
    const getDetail = () => {
      priceSettingApi.getDetail(selectId).then((res) => {
        if (res && res.code == 0) {
          ...
          getAddressList(res.data.customerId)
        }
      });
    };
    selectId && getDetail();
    ...
    const delMaterial = (ids) => {
      ElMessageBox.confirm(
        ...
        state.tableDataFrom.tableData = transformArrayData(tableDataList, true, true);
        console.log('state.tableDataFrom.tableData',state.tableDataFrom.tableData)
      });
    };
   ...
   const disabledEndDate = (time)=>{
     if(state.formData.startExpireTime){
       return time.getTime() < moment(state.formData.startExpireTime).format('x')
     }else{
       return time.getTime() < Date.now() - 8.64e7
     }
   }
      ...
      console.log(row)
      state.tableDataFrom.tableData[row.key].price=row.productPrice+row.transportPrice
    }
        const onModalClose = () => {
     ...
     const getSelected = (val) => {
      ...
      state.tableDataFrom.tableData = transformArrayData(materialData, true, true);
    }
      const selectedCustomer = (val) => {
      state.formData.customerId = val.id;
      state.formData.customerCode = val.code;
