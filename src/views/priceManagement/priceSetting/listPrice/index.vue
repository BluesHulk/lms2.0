<template>
    ...
    <div class="content">
      ...
      <template v-if="currentRow.id && !operatingRecordShow">
        <div class="table-header">
          <el-button @click="allocaCustomer" class="operating-record-btn"
            >分配客户</el-button
          >
          <div class="title">
        ...
        <MyTable
          ...
          :selection="true"
          @handleSelectionChange="detailHandleSelectionChange"
        />
      </template>
  ...
  <CustomerSelect
    ...
    :visible="customerModelShow"
    @onTreeChange="customerChange"
    @customerClose="customerClose"
  >
    <template v-slot:header>
      <div>
        已选中{{
          detailSelectRows.length
        }}条物料价格数据，请选择分配客户(多选)：
      </div>
...
export default {
  ...
  setup() {
    ...
    const customerModel = reactive({
      customerModelShow: false,
      customerTreeData: [],
    });
    // 获取客户
    const getCustomer = () => {
      commonApi.getCustomerList().then((res) => {
        customerModel.customerTreeData = res.data.map((item) => {
    ...
    getCustomer();
    //分配客户
    const allocaCustomer = () => {
      if (detailTableRecord.detailSelectRows.length) {
        customerModel.customerModelShow = true;
      } else {
        ElMessage.warning("请至少选择1项");
      }
    };
    const customerChange = (item) => {
      let ids = [];
      detailTableRecord.detailSelectRows.map((item) => {
        ids.push(item.id);
      });
      const params = {
        priceListGoodsIds: ids,
      };
      params.priceListCustomerQos = item.map((v) => {
        return {
          customerCode: v.code,
          customerId: v.id,
          customerName: v.name,
          customerShortName: v.shortName,
          customerCategoryName: v.customerCategoryName,
          customerCategoryId: v.customerCategoryId,
          customerSalesType: v.saleCustomerCategoryId,
          customerSalesTypeName: v.saleCustomerCategoryName,
        };
      });
      priceSettingApi.addCustomer(params).then((res) => {
        if (res && res.code == 0) {
          ElMessage.success("分配成功！");
          customerModel.customerModelShow = false;
        } else {
          ElMessage.error(res.message);
        }
      });
    ...
    const detailHandleSelectionChange = (val) => {
      detailTableRecord.detailSelectRows = val;
    };
    ...
    const exportOut = () => {
      ...
      ElMessageBox.confirm(
        ...
        .then(async () => {
          ...
          const res = await priceSettingApi.download(ids, 1);
          let fileName = "价目表";
          const _res = res;
