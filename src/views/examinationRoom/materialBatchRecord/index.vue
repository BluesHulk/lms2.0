...
export default {
  ...
  setup() {
    ...
    const getTableData = () => {
      ...
      batchRecordApi.getTableData(params).then((res) => {
        if (res && res.code == 0) {
    ...
    const onStatusChange = (record) => {
      ...
      ElMessageBox.confirm(
        `<strong>提示</strong><br/>
        您确定要对物料【${record.batchNum}】${{ 0: "启用", 1: "禁用" }[status]}批次吗？`,
        {
        ...
        batchRecordApi.onStatusChange({ id: record.id, status })
          .then(res => {
            if(res && res.code == 0) {
    ...
    const onDelete = (record) => {
      ElMessageBox.confirm(
        ...
        batchRecordApi.onDelete(record.id).then((res) => {
          if (res && res.code == 0) {
            ElMessage.success("操作成功！");
            getTableData();
          } else {
    ...
    const onModalShow = (type, record = {}) => {
      ...
      record.batchType && (modalRecord.modalData.batchType = record.batchType * 1);
    };
    ...
    const tableColumns = [
      ...
      { label: "当前批号", prop: "batchNum", width: 140 },
      { label: "检测量(吨)", prop: "amount", width: 100 },
      { label: "已发量(吨)", prop: "useAmount", minWidth: 100 },
      {
        label: "剩余量(吨)",
        prop: "",
        width: 100,
        formatter: (row) => (
          (row.amount - (row.useAmount || 0)).toFixed(2)
        )
      },
      { label: "预警量(%)", prop: "warningAmount", width: 100 },
      { label: "禁超量(%)", prop: "banSurpassAmount", width: 100 },
